...
/// information.
class MultipleRoundedPointsClipperCustom extends CustomClipper<Path> {
  final double heightOfPoint;
  final int numberOfPoints;
  final Sides side;
  final double dotSize;
  MultipleRoundedPointsClipperCustom(this.side, {this.heightOfPoint = 12, this.numberOfPoints = 16, this.dotSize = 8}); // final Sides side;
  ...
  Path getClip(Size size) {
    ...
    ///// Oval
    path.addOval(Rect.fromCircle(center: Offset(0.0, size.height / 3.5), radius: 12.0));
    path.addOval(Rect.fromCircle(center: Offset(size.width, size.height / 3.5), radius: 12.0));
    ///// Oval
    path.close();
    return path;
  }
...
class Dot {
  ...
  Dot({this.size = 0, this.column = 20, this.row = 0});
